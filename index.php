...
function multiplicarPorHorarios($filmes, $inicioTagHorarios, $finalTagHorarios) {
    for ($i = 0; $i < count($inicioTagHorarios); $i++) {
        $contador = 0;
        $filmesMultiplicados = [];
        $finalTagHorarios[$i] = str_replace('/', '\/', $finalTagHorarios[$i]);
        for ($z = 0; $z < count($filmes); $z++) {
            preg_match('/' . $inicioTagHorarios[$i] . '(.*)' . $finalTagHorarios[$i] . '/s', $filmes[$z], $horariocompleto);
            preg_match_all('/' . $inicioTagHorarios[$i] . '(.*)' . $finalTagHorarios[$i] . '/Us', $filmes[$z], $horarios, PREG_SET_ORDER);
...
function cadastrarFilme($nome, $classifEtaria) {
    ...
    if ($tbl = mysqli_fetch_array($result)) {
        ...
        if ($iguais != false) {
            $indice = $iguais[0];
            for ($a = 1; $a < count($iguais); $a++) {
                if ($saida[$indice]->release_date < $saida[$iguais[$a]]->release_date ) {
                    $indice = $iguais[$a];
                }
            }
        }
...
function cadastrarLocais($endereco) {
    ...
    if ($tbl = mysqli_fetch_array($result)) {
        ...
        if ($tbl = mysqli_fetch_array($result)) {
            ...
            $result = mysqli_query($link, $sql);
            if ($result) {
                echo "<br>Cadastro de Local efetuado com sucesso!";
            } else {
                echo("<br>Error description: " . mysqli_error($link));
            }
...
function executar($endereco, $linkPrecos, $link, $organizacaoSite, $identificadorData, $identificadorFilme, $inicioHorario, $finalHorario, $inicioNome, $finalNome, $inicioClassificacao, $finalClassificacao, $inicioExibicao, $finalExibicao, $identificador3d, $listaExtras, $horarioInicioNoite, $listaExpressoes, $listaTipos, $feriados, $codificacao) {
    $buscaPrecos = buscarHTML($linkPrecos);
    ...
    $listaPrecos = criarListaDePrecos($listaExpressoes, $listaTipos, $buscaPrecos);
    print_r($listaPrecos);
    for ($a = 0; $a < count($dias); $a++) {
        ...
        for ($b = 0; $b < count($dias[$a]); $b++) {
            ...
            $clasEtariaDoFilme = 0;// acharClassificacaoEtaria($dias[$a][$b], $inicioClassificacao, $finalClassificacao);
            ...
            $filmesdia[$b][4] = acharHorario($dias[$a][$b], $inicioHorario[count($inicioHorario) -1], $finalHorario[count($finalHorario) -1]);
...
executar($enderecoCinemarkAracajuJardins, 'http://www.cinemark.com.br/programacao/aracaju/shopping-jardins/10/706', 'http://www.ingresso.com/aracaju/home/local/cinema/cinemark-shopping-jardins-aracaju', 1, '<div id="outrosLocais_', '<div id="local1', ['<tr ', 'class="btTxt">'], ['</tr>(.*)</table>(.*)</td>', '</span></a>'], 'class="nomeEspetaculo">', '</a>', 'censura\/censura', '.png', 'tipo-legenda" title="', '">', "title='3D'", false, 1656, $listaExpressoes, $listaTipos, $feriados, "UTF-8");
executar($enderecoCinemarkNatal, 'http://www.cinemark.com.br/programacao/natal/midway-mall-natal/22/681', 'http://www.ingresso.com/natal/home/local/cinema/cinemark-natal', 1, '<div id="outrosLocais_', '<div id="local1', ['<tr ', 'class="btTxt">'], ['</tr>(.*)</table>(.*)</td>', '</span></a>'], 'class="nomeEspetaculo">', '</a>', 'censura\/censura', '.png', 'tipo-legenda" title="', '">', "title='3D'", false, 1656, $listaExpressoes, $listaTipos, $feriados, "UTF-8");
//executar($enderecoCinemarkAracajuJardins, 'http://www.cinemark.com.br/programacao/aracaju/shopping-jardins/10/706', 0, '<div id="outrosLocais_', '<div id="local1', '<span id="HH_706_(.*)">', '</span>(?=</p>)?', 'class="nomeEspetaculo">', '(?=( 3D)?(&nbsp;)?</a>)', 'censura\/censura', '.png', 'exibicao..jpg" title="', '" alt', 'alt="3D"', false, 1701, $listaExpressoes, $listaTipos, $feriados, "UTF-8");
$d2e3 = 'Segunda(.*)';
$d4 = 'Quarta(.*)R\$ (.*) \(inteira';
$d5eEtc = 'Quinta(.*)';
$listaExtras = [[4, "title='VIP'", 'VIP'], [8, "title='Macro XE'", 'MacroXE']];
...
executar($enderecoCinepolisNatalShopping, 'http://www.cinepolis.com.br/programacao/cinema.php?cc=31', 'http://www.ingresso.com/natal/home/local/cinema/cinepolis-natal-shopping', 1, '<div id="outrosLocais_', '<div id="local1', ['<tr ', 'class="btTxt">'], ['</tr>(.*)</table>(.*)</td>', '</span></a>'], 'class="nomeEspetaculo">', '</a>', 'censura\/censura', '.png', 'tipo-legenda" title="', '">', "title='3D'", $listaExtras, 1656, $listaExpressoes, $listaTipos, $feriados, "UTF-8");
/*
